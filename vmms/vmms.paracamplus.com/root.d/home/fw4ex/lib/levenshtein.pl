...
=pod
    # QNC pour voir ce qui se passe
    print STDERR 's1 is: ' . printable($s1) . "\n";
    print STDERR 's2 is: ' . printable($s2) . "\n";
    my $line = "\t";
    for (my $j = 1; $j <= $len2; ++$j) {
        $line .= "(" . printable($ar2[$j-1]) . ")\t";
    }
    print STDERR $line, "\n";
    for (my $i = 1; $i <= $len1; ++$i ) {
        $line = "(" . printable($ar1[$i-1]) . ")\t";
        for (my $j = 1; $j <= $len2; ++$j) {
            $line .= $mat{$i}{$j} . "\t";
        }
        print STDERR $line, "\n";
