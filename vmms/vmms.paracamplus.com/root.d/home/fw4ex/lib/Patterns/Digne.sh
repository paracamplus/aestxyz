...
# command will be run in various directories.

# This pattern gauges the student's answer by comparison to the
# author's solution. The command is run within various directories,
# the result is in the stdout. To sum up and using the variables
...
# This variable specifies the additional options to run C<COMMAND>. By
...
# solution that is a script achieving the desired behavior. By
