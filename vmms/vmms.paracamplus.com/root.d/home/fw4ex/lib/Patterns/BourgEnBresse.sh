...
# the stem. For instance, what options to the Unix command C<tr> may
# convert lower cases letters to upper case letters ? An answer is
# C<a-z A-Z> but not C<tr a-z A-Z> since the options are required but
# not the command (C<tr>) which is already known.
...
# This variable if set tells how to transform the data to be fed to
# COMMAND. Examples of PRECOMMAND may be C<sort> or C<cut | sort>.
