...
# This pattern corresponds to an exercise that asks for a command. For
# instance, what command may convert the lower cases letters from the
# file mentioned in its first argument to upper case letters ?

# This pattern gauges the student's answer by comparison to the
# author's solution. The command receives data file as argument, the
...
# script to run. By default, this file is named C<command>. This
# command will receive the name of the data file as an argument
# surrounded with LEFT_FLAGS and RIGHT_FLAGS.
...
# displayed or not. Byt default, the script is displayed.
