...
# Slightly modified on 2012 May 20 by QNC

# Insert in crontab:
# (export WGET_FLAGS=--no-check-certificate ; send_nrdp.sh -n -t "xyzXYZ" -u https://xxyy.zz -H youpouLip6 -s FW4EX-VM -S 0 -o "OK|`date`" )
# Now, use the -n option to replace that export WGET_FLAGS
...
send_data() {
    ...
    if [ ! "x$curl" == "x" ];then
        ...
        rslt=`wget ${WGET_FLAGS} -q -O - --post-data="$pdata" "$url/"`
        ret=$?
    fi
...
while getopts "u:t:H:s:S:o:f:d:c:D:hvn" option
do
  case $option in
    u) url=$OPTARG ;;
    t) token=$OPTARG ;;
    ...
    v) print_release
        exit 0 ;;
    n) WGET_FLAGS=--no-check-certificate;;
  esac
done
...
# QNC: SKIP this part, it interacts badly with crontab !?
# Detect STDIN
########################
if false ; then #[ ! -t 0 ]; then
    xml=""
